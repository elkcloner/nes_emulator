...
static InterruptLines interrupts;

int cpu_run(int maxCycles) {
	int count = 0;

	while ((count += cpu_exec()) < maxCycles) ;
...
int cpu_exec() {
	...
	if (!interrupts.isr) {
		...
		if (interrupts.reset) {
			...
			set_sp(get_sp()-1);
			// Set interrupt disable flag and isr flag
			set_iFlag();
			interrupts.isr = 1;
			...
			state.pc = mem_read_cpu(0xfffb);
			state.pc <<= 8;
			state.pc |= mem_read_cpu(0xfffa);
			cycles = 7;
		} else if (interrupts.irq) {
			// TODO Currently only generated by BRK instruction which already
			// pushes the pc and status registers
			// Clear interrupt line
			interrupts.reset = 0;
			// Set interrupt disable flag and isr flag
			set_iFlag();
			interrupts.isr = 1;
			// Set PC to reset vector
			state.pc = mem_read_cpu(0xffff);
			state.pc <<= 8;
			state.pc |= mem_read_cpu(0xfffe);
		}
	}
...
int cpu_interrupt(Interrupt type) {
	switch(type) {
		case RESET:
			interrupts.reset = 1;
			break;
